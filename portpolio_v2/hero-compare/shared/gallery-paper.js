...
async function createPaper(T) {
  ...
  const geometry = new T.PlaneGeometry(1,1,40,80);
  const vertexShader = `
    uniform vec2 uSize;
    uniform float uTime, uBend, uIdle, uTravel, uCurl;
    varying vec2 vUv;
    ...
    void main() {
      ...
      vec3 p=position;
      p.xy*=uSize;
      // A moving curl boundary: upper paper is flat; the lower sheet wraps around a cylinder.
      float flatLength=uSize.y*(1.-uCurl);
      float distanceFromTop=(1.-uv.y)*uSize.y;
      float rolledLength=max(0.,distanceFromTop-flatLength);
      float radius=uSize.y*.165;
      float angle=rolledLength/radius;
      if(rolledLength>0.) {
        p.y=uSize.y*.5-flatLength-radius*sin(angle);
        p.z=radius*(1.-cos(angle));
      }
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      vec3 paper=gl_FrontFacing?tex.rgb:mix(vec3(.86,.84,.79),tex.rgb,.07);
      gl_FragColor=vec4(paper*vLight,tex.a*uOpacity);
  ...
  try {
    for (const media of [...desktopMedia,...mobileMedia]) {
      ...
      const material=new T.ShaderMaterial({vertexShader,fragmentShader,transparent:true,depthWrite:true,side:T.DoubleSide,uniforms:{
        uMap:{value:null},uSize:{value:new T.Vector2()},uTime:{value:0},uBend:{value:0},uIdle:{value:0},uTravel:{value:0},uOpacity:{value:0},uCurl:{value:0}
      }});
      const mesh=new T.Mesh(geometry,material);
      mesh.frustumCulled=false; scene.add(mesh);
      records.push({media,img,key,desktop,mesh,material,texture:null,textureWidth:0,visible:false,entered:false,everShown:false,curl:0,opacity:0,y:0,bend:0,rotation:0,scale:1,transition:null});
    }
  ...
  function transition(r,enter,h) {
    const opening=enter&&!r.everShown;
    if(enter&&!r.opacity) {r.y=h*.31;r.bend=opening?12:95*(r.desktop?1:.55);r.rotation=-.09;r.scale=opening?1:.95;}   // Claude polish 2: 새 화면은 조금 작게 시작해 앞으로
    if(opening)r.curl=.98;
    r.transition={start:elapsed,from:[r.opacity,r.y,r.bend,r.rotation,r.scale],to:enter?[1,0,0,0,1]:[0,-h*.27,-55,-.07,.88],curlFrom:r.curl,opening,duration:opening?(r.desktop?1.65:1.35):enter?1.08:.72};   // 이전 화면은 뒤로 물러나며(.88) 사라진다
    if(enter)r.everShown=true;
  ...
  function draw(now) {
    ...
    for(const r of records) {
      ...
      if(r.transition) {
        ...
        [r.opacity,r.y,r.bend,r.rotation,r.scale]=tr.from.map((v,i)=>v+(tr.to[i]-v)*ease);
        r.curl=tr.curlFrom*(1-p*p*(3.-2.*p));
        if(tr.opening)r.opacity=Math.min(1,p*7.);
      ...
      u.uCurl.value=r.curl;
      u.uIdle.value=(r.key==='arpina'?7:10)*(r.desktop?1:.55);
      // Claude polish 3: 목록이 올라오는 동안(--arrive 0→1) 판이 .9→1로 자리 잡고 선명해진다
      const arrive=r.desktop?(parseFloat(gallery.style.getPropertyValue('--arrive'))||1):1;
      u.uOpacity.value=r.opacity*(.5+.5*arrive);
      const ms=r.scale*(.9+.1*arrive); r.mesh.scale.set(ms,ms,1);
