...
async function createPaper(T) {
  ...
  try {
    for (const media of [...desktopMedia,...mobileMedia]) {
      ...
      records.push({media,img,key,desktop,mesh,material,texture:null,textureWidth:0,visible:false,entered:false,everShown:false,curl:0,opacity:0,y:0,bend:0,rotation:0,transition:null});
    }
  ...
  function transition(r,enter,h) {
    ...
    if(enter&&!r.opacity) {r.y=h*.31;r.bend=opening?12:95*(r.desktop?1:.55);r.rotation=-.09;}
    if(opening)r.curl=.98;
    r.transition={start:elapsed,from:[r.opacity,r.y,r.bend,r.rotation],to:enter?[1,0,0,0]:[0,-h*.27,-55,-.07],curlFrom:r.curl,opening,duration:opening?(r.desktop?1.65:1.35):enter?1.08:.72};
